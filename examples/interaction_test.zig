...
fn onHover(data: *anyopaque, _: lu.Offset, enter: bool) void {
...
/// `ctx.events.isDragged(id)` + `ctx.events.pointer` during its build.
fn onDrag(data: *anyopaque, _: lu.Offset, active: bool) void {
    const s: *TestState = @ptrCast(@alignCast(data));
...
fn onFocus(data: *anyopaque, _: lu.FocusSource, gained: bool) void {
    const s: *TestState = @ptrCast(@alignCast(data));
    if (gained) s.focus_gain += 1 else s.focus_lose += 1;
}
...
fn reset(window: *lu.Window, s: *TestState) void {
    s.* = .{};
    const events = &window.ctx.?.events;
    events.reset();
    events.pointer = .{ .x = 0, .y = 0 };
    events.pointer_inside = false;
...
pub fn main() !void {
    ...
    const box = ctx.box(.{ .w = 300, .h = 100 }, .{
        ...
        .events = .{
            ...
            .focus = .{ .handle = .{ .fptrs = &.{.{ .data = &state, .func = &onFocus }} } },
        },
    }, @src());
    const box_id = box.id;
    root.layout.?.end();
    ...
    check(ctx.events.isClicked(box_id) != null, "A view clicked is box");
    check(ctx.events.isClicked(box_id).?.x == 10 and ctx.events.isClicked(box_id).?.y == 10, "A click local offset");
    ...
    check(ctx.events.isClicked(box_id) == null, "A view clicked cleared");
    ...
    check(ctx.events.isClicked(box_id) == null, "B view clicked cleared");
    ...
    check(ctx.events.isDragged(box_id) != null, "C view dragged is box");
    pushMotion(win_id, 100, 10);
    frame(&window, &root);
    check(state.drag_activates == 1, "C no streamed drag calls");
    check(ctx.events.isDragged(box_id) != null, "C drag persists");
    ...
    check(ctx.events.isDragged(box_id) == null, "C view dragged cleared");
    ...
    // ---------- Scenario E: focus follows clicks and clears on empty press. ----------
    reset(&window, &state);
    box.events.drag = .{ .handle = null };
    pushButton(win_id, 10, 10, true);
    frame(&window, &root);
    check(state.focus_gain == 1, "E focus gained on click");
    check(ctx.events.isFocused(box_id) == .mouse, "E view focused reports mouse source");
    pushButton(win_id, 10, 10, false);
    frame(&window, &root);
    check(state.focus_lose == 0, "E release keeps focus");
    pushButton(win_id, 400, 400, true);
    frame(&window, &root);
    check(state.focus_lose == 1, "E empty press drops focus");
    check(ctx.events.isFocused(box_id) == null, "E view focused cleared");
